...
long int* fquoteIndices(int numQuotes, long int* Indices);					// Function returns an array that indicates the start of every quote in the file (number of characters from the start of the file) 
int* fquoteLength(int numQuotes, long int* Indices, int *LengthMessage); // Function returns the smaller of the actual quote length or MAX_QUOTE_LENGTH
