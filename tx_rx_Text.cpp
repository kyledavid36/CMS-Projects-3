...
extern COMMTIMEOUTS timeout;						// A commtimeout struct variable
 ...
 int tx_rx_Text() {
	...
	printf("Enter (1) for Transmit (2) for Receive: ");
	selection = getchar();
	...
	if (selection == '1') {
		...
		txHeader.sid = 1;											
		txHeader.rid = 2;
		txHeader.payloadSize = strlen(txPayload) + 1;				//(audio lBigBufSize*2)  Flexible payload size - Send size of payload inside header (payload can be anything) and enough memory will be malloc'd in the receive function
